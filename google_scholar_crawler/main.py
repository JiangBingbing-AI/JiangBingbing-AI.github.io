...
def parse_serpapi_metrics(payload: dict) -> dict:
    ...
    for metric_name, fields in field_map.items():
        ...
        recent_key = next((key for key in values if key.startswith("since_")), None)
        for key, field in zip(("all", recent_key), fields):
            if key in values:
...
def main() -> None:
    scholar_id = os.environ.get("GOOGLE_SCHOLAR_ID", DEFAULT_GOOGLE_SCHOLAR_ID).strip()
    if "://" in scholar_id:
        scholar_id = parse_qs(urlparse(scholar_id).query).get("user", [""])[0]
    scholar_id = scholar_id.split("&", 1)[0].strip()
    if not scholar_id:
        raise ValueError(
